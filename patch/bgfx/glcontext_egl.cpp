...
#	endif // BX_PLATFORM_WINDOWS
			...
			bool useGbmDisplay = false;
#	if BX_PLATFORM_LINUX || BX_PLATFORM_BSD
			const char* useGbmEnv = getenv("BGFX_USE_GBM");
			useGbmDisplay = NULL != useGbmEnv && '1' == useGbmEnv[0];
			...
			&&  NULL != ndt)
			{
				...
				if (NULL == eglGetPlatformDisplayExt)
				{
					eglGetPlatformDisplayExt = (PFNEGLGETPLATFORMDISPLAYEXTPROC)eglGetProcAddress("eglGetPlatformDisplay");
				}

				if (NULL != eglGetPlatformDisplayExt)
				{
					m_display = eglGetPlatformDisplayExt(EGL_PLATFORM_GBM_KHR, (void*)ndt, NULL);
				}
			}
#	else
			m_display = EGL_NO_DISPLAY;
...
#undef GET_CONFIG_ATTRIB
			...
			for (uint32_t retry = 0; retry < 2; ++retry)
			{
				...
				if (useGbmDisplay)
				{
					...
					attrs[numAttrs++] = EGL_DONT_CARE;
				}

				attrs[numAttrs++] = EGL_BLUE_SIZE;
				attrs[numAttrs++] = useGbmDisplay ? 0 : colorBlockInfo.bBits;

				attrs[numAttrs++] = EGL_GREEN_SIZE;
				attrs[numAttrs++] = useGbmDisplay ? 0 : colorBlockInfo.gBits;

				attrs[numAttrs++] = EGL_RED_SIZE;
				attrs[numAttrs++] = useGbmDisplay ? 0 : colorBlockInfo.rBits;

				attrs[numAttrs++] = EGL_ALPHA_SIZE;
				attrs[numAttrs++] = useGbmDisplay ? 0 : colorBlockInfo.aBits;

				attrs[numAttrs++] = EGL_DEPTH_SIZE;
				attrs[numAttrs++] = useGbmDisplay ? 0 : depthStecilBlockInfo.depthBits;

				attrs[numAttrs++] = EGL_STENCIL_SIZE;
				attrs[numAttrs++] = useGbmDisplay ? 0 : depthStecilBlockInfo.stencilBits;
				...
				EGLConfig chosenConfigs[256];
				success = eglChooseConfig(m_display, attrs, chosenConfigs, BX_COUNTOF(chosenConfigs), &numConfigs);
				...
				m_config = chosenConfigs[0];

#	if BX_PLATFORM_LINUX || BX_PLATFORM_BSD
				if (useGbmDisplay
				&&  !headless
				&&  EGLNativeWindowType(0) != nwh)
				{
					...
					if (NULL != eglCreatePlatformWindowSurfaceEXT)
					{
						for (EGLint cfgIdx = 0; cfgIdx < numConfigs; ++cfgIdx)
						{
							EGLSurface probeSurface = eglCreatePlatformWindowSurfaceEXT(m_display, chosenConfigs[cfgIdx], (void*)nwh, NULL);
							if (EGL_NO_SURFACE != probeSurface)
							{
								m_config = chosenConfigs[cfgIdx];
								eglDestroySurface(m_display, probeSurface);
								break;
							}
						}
					}
...
#	if BX_PLATFORM_LINUX || BX_PLATFORM_BSD
				if (useGbmDisplay)
				{
					...
					if (NULL != eglCreatePlatformWindowSurfaceEXT)
					{
						m_surface = eglCreatePlatformWindowSurfaceEXT(m_display, m_config, (void*)nwh, NULL);
					}
				}
#	endif // BX_PLATFORM_LINUX || BX_PLATFORM_BSD
