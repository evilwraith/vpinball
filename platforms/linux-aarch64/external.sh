...
if [ "${BGFX_EXPECTED_SHA}" != "${BGFX_FOUND_SHA}" ]; then
   ...
   # FORK PATCH (4kp): let VPX present the primary surface itself. eglSwapBuffers waits for the whole
   # context's GPU work, which serialises the frame when something else owns presentation.
   ( cd bgfx && patch -p1 < "${VPX_PATCH_DIR}/bgfx-skip-primary-swap.patch" )
