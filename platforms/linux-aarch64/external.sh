...
if [ "${BGFX_EXPECTED_SHA}" != "${BGFX_FOUND_SHA}" ]; then
   ...
   # FORK PATCH (4kp): runtime bgfx_set_skip_present() so VPX can own presentation (owned scanout
   # + IN_FENCE_FD) without bgfx's eglSwapBuffers serialising the frame. Applies after the GBM patch.
